...
description: "Run evaluation workflows in batch before release."
---
...
For the complete usage guide with code examples, see [Offline evaluation](/scouter/agents/offline-evaluation/).

> For how these building blocks (task types, DAGs, operators) work, see [Building blocks](/scouter/evaluation-platform/eval-profiles-and-tasks/).
...
sequenceDiagram
    ...
    loop each scenario
        ...
        A->>Q: span.attach_eval(profile_uid, context, ...)
...
During scenario execution, your agent emits `EvalRecord` objects through the queue. Inside a traced span, you call `span.attach_eval(profile_uid, context, ...)`, and the record is automatically stamped with the active span's `trace_id` and `span_id`. This connection allows `TraceAssertionTask` to match records to spans during evaluation.

The queue is in capture mode, so records accumulate in an in-memory buffer. After each scenario completes, the orchestrator calls `drain_all_records()`, which returns a list of `EvalRecord` objects. These records, along with the agent's response, are passed to the Rust engine via `collect_scenario_data()`.

Trace spans follow a similar path. The orchestrator enables a run-scoped local span capture buffer at the start of `run()`. Spans accumulate there during scenario execution. After all scenarios complete, `flush_tracer()` ensures every span has landed in that run's buffer before evaluation begins.

The Rust engine matches records to spans by `trace_id` during evaluation. It uses the span's `trace_id` and `span_id` to resolve `TraceAssertionTask` definitions against the captured span data.

```mermaid
graph LR
    subgraph "During scenario execution"
        AF["agent_fn()"] --> S["Traced span"]
        S --> QI["span.attach_eval(profile_uid, context)"]
        QI --> TID["record.trace_id<br/>record.span_id<br/>(set by span constructor)"]
        S --> SB["Span buffer"]
    end

    subgraph "After all scenarios"
        DR["drain_all_records()"] --> RR["Records (with trace_id)"]
        SB --> FT["flush_tracer()"]
        FT --> SS["Spans (with trace_id)"]
    end
...
For details, see [EvalDataset](/scouter/agents/eval-dataset/).
