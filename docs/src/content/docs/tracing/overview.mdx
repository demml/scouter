...
description: "Use Scouter as an OpenTelemetry tracer provider for AI services."
---
...
Traces captured by Scouter can be evaluated offline or in production using [`TraceAssertionTask`](/scouter/agents/tasks/#traceassertiontask) — validating span execution order, latency SLAs, token budgets, and more.
...
````
  ...
  <TabItem label="Scouter + OTEL collector">
...
````
  ...
  <TabItem label="Low-level manual setup">
...
Full API reference, framework integration examples (OpenAI Agents SDK, LangChain, LlamaIndex, CrewAI, FastAPI), default attributes, and local span capture patterns are covered in the [ScouterInstrumentor guide](/scouter/tracing/instrumentor/).
...
`ScouterInstrumentor` uses the Rust tracing core and registers Scouter as the process-wide OTEL provider:

| Capability | `ScouterInstrumentor` |
|--|----------------------|
| Registers global OTEL provider | Yes |
| Works with OTEL auto-instrumentation libraries | Yes |
| Recommended for application/runtime code | Yes |
| Follows OTel `BaseInstrumentor` lifecycle | Yes |

Use `ScouterInstrumentor` for application code and tests, especially when third-party OTEL libraries are involved.
...
<Tabs>
  <TabItem label="Client (orchestrator)">
  </TabItem>
</Tabs>
````python
...
<Tabs>
  <TabItem label="Server (downstream service)">
  </TabItem>
</Tabs>
````python
...
def monitoring_task(app: FastAPI):
    ...
    with tracer.start_as_current_span("monitoring_task") as span:
        # attach evaluation record to trace
        span.attach_eval(
            profile_uid="production/agent/1.0.0",  # (1)
            context={...},                          # (2)
            record_id="turn-1",
...
1. Profile UID (space/name/version triple)
2. Evaluation context as a dict or Pydantic model
...
:::note
Exporter-level `sample_ratio` only affects the optional OTEL exporter. To enforce the same ratio for both Scouter and OTEL export, set `sample_ratio` on `instrument()` directly.
:::
