...
Traces captured by Scouter can be evaluated offline or in production using [`TraceAssertionTask`](/agents/tasks/#traceassertiontask) — validating span execution order, latency SLAs, token budgets, and more.
...
Full API reference, framework integration examples (OpenAI Agents SDK, LangChain, LlamaIndex, CrewAI, FastAPI), default attributes, and local span capture patterns are covered in the [ScouterInstrumentor guide](/tracing/instrumentor/).
