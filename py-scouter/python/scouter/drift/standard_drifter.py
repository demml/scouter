...
# from .custom import CustomMetricData  # CustomDriftHelper,
...
class StandardDrifter:
    ...
    def create_drift_profile(
    ...
    ) -> Union[Profile, CustomDriftProfile]:
        ...
        # if _config.drift_type == DriftType.Custom:
        #     if isinstance(_config, CustomMetricDriftConfig):
        #         if isinstance(data, CustomMetricData):
        #             self._custom_drift_helper.create_custom_profile(data, _config)
        #         raise ValueError(
        #             f"Invalid custom metric drift data detected, expected CustomMetricData, but found {type(data)}"
        #         )
        #     raise ValueError(f"Invalid configuration found for custom metric drift type {type(_config)}")
        # if isinstance(_config, (SpcDriftConfig, PsiDriftConfig)):
        #     return self._drift_helper.create_drift_profile(data, _config)
        # raise ValueError("jhs")
        return self._drift_helper.create_drift_profile(data, _config)
