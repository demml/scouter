...
def test_data_profile_polars_mixed_type(
    ...
    assert profile.features["cat_feature_0"].string_stats.word_stats.words[
        "3"
    ].percent == pytest.approx(0.323, abs=0.1)

    assert profile.features["cat_feature_1"].string_stats.distinct.count == 5
    assert profile.features["cat_feature_1"].string_stats.word_stats.words[
        "7"
    ].percent == pytest.approx(0.22, abs=0.1)
