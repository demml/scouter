...
def test_data_profile_f64(array: NDArray):
    ...
    assert profile.features["feature_0"].numeric_stats.distinct.count == pytest.approx(1000, 1)
    assert profile.features["feature_0"].numeric_stats.quantiles.q25 == pytest.approx(1.25, 0.1)
    assert profile.features["feature_0"].numeric_stats.histogram.bins[0] == pytest.approx(1.00, 0.1)
    ...
    assert loaded_profile.features["feature_0"].numeric_stats.mean == pytest.approx(1.5, 0.1)
...
def test_data_profile_polars(array: NDArray):
    ...
    assert profile.features["column_0"].numeric_stats.distinct.count == pytest.approx(1000, 1)
    assert profile.features["column_0"].numeric_stats.quantiles.q25 == pytest.approx(1.25, 0.1)
    assert profile.features["column_0"].numeric_stats.histogram.bins[0] == pytest.approx(1.00, 0.1)
...
def test_data_profile_pandas(array: NDArray):
    ...
    assert profile.features["0"].numeric_stats.histogram.bins[0] == pytest.approx(1.00, 0.1)
