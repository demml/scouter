use pyo3::prelude::*;
use scouter_client::{
    disable_local_capture, drain_local_spans, enable_local_capture, flush_tracer,
...
pub fn add_tracing_module(m: &Bound<'_, PyModule>) -> PyResult<()> {
    ...
    m.add_function(wrap_pyfunction!(enable_local_capture, m)?)?;
    m.add_function(wrap_pyfunction!(disable_local_capture, m)?)?;
    m.add_function(wrap_pyfunction!(drain_local_spans, m)?)?;
    Ok(())
}
