...
- `LLMJudgeTask` — LLM-powered semantic evaluation. Injects context variables into `Prompt`. Supports OpenAI, Anthropic, Google. Uses structured output (Pydantic models). `context_path` extracts field from LLM response. Multimodal judge tasks support static media baked into the prompt via potatohead `MediaRef` and dynamic per-record media via `EvalRecord(media=[ImageMedia(id="chart", url=...)])`, referenced with isolated `${media:chart}` placeholders. Sources are URL (provider fetches), raw `bytes`, or local `path` read at construction; this is distinct from `${var}` text substitution. Prompt retry-on-schema-mismatch is not active for multimodal judge tasks in v1.
