...
pub use scouter_types::observability_contract;
...
mod server_otlp_tests {
    ...
    async fn enabled_server_otlp_exports_spans_to_configured_grpc_collector() {
        ...
        let settings = OtelSettings {
            ...
            protocol: ScouterOtelProtocol::Grpc,
            service_name: "scouter-server-test".to_string(),
            sample_ratio: 1.0,
            export_timeout_secs: 2,
        };

        let handle = init_server_otlp_tracing(&settings)
            .unwrap()
            .expect("enabled OTLP settings should build a handle");
        let tracer = handle.tracer();
        let mut span = tracer.start("server.otlp.enabled.test");
        span.end();
        handle.force_flush().unwrap();

        for _ in 0..20 {
            if !collector.requests.lock().await.is_empty() {
                break;
            }
        ...
        assert!(
            requests
                .iter()
                .any(|request| !request.resource_spans.is_empty()),
            "expected the in-process OTLP collector to receive exported spans"
        );
        drop(requests);
