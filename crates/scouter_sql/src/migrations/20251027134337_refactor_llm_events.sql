-- Refactoring llm_drift_record to llm_event_record table + adding tracing fields

BEGIN;

-- Rename existing table to preserve data
ALTER TABLE scouter.llm_drift_record RENAME TO llm_event_record;

-- Add new tracing and event columns
ALTER TABLE scouter.llm_event_record 
...
ADD COLUMN IF NOT EXISTS entity_type text DEFAULT 'llm';
...
ALTER TABLE scouter.llm_event_record DROP COLUMN IF EXISTS context;

-- Remove old constraints before adding new ones
ALTER TABLE scouter.llm_event_record DROP CONSTRAINT IF EXISTS llm_drift_record_pkey;
ALTER TABLE scouter.llm_event_record DROP CONSTRAINT IF EXISTS llm_drift_record_created_at_name_space_version_key;

-- Create new primary key and constraints
ALTER TABLE scouter.llm_event_record 
ADD CONSTRAINT llm_event_record_pkey PRIMARY KEY (uid, created_at),
ADD CONSTRAINT llm_event_record_unique_span UNIQUE (span_id, created_at),
ADD CONSTRAINT llm_event_record_entity_type_check CHECK (entity_type IN ('llm', 'retrieval', 'embedding', 'classification'));
...
CREATE INDEX idx_llm_event_record_space_name_version_time
ON scouter.llm_event_record (space, name, version, created_at DESC);

-- Critical for trace reconstruction - groups spans by trace
CREATE INDEX idx_llm_event_record_trace_reconstruction
ON scouter.llm_event_record (trace_id, created_at)
INCLUDE (span_id, span_name, parent_span_name, entity_type);

-- Span hierarchy navigation - find children of a span
CREATE INDEX idx_llm_event_record_span_hierarchy  
ON scouter.llm_event_record (parent_span_name, created_at)
WHERE parent_span_name IS NOT NULL;

-- Root span identification - entry points into traces
CREATE INDEX idx_llm_event_record_root_spans
ON scouter.llm_event_record (trace_id, created_at)
WHERE parent_span_name IS NULL;

-- Background processing - pending records
CREATE INDEX idx_llm_event_record_status_processing
ON scouter.llm_event_record (status, created_at ASC)
WHERE status IN ('pending', 'processing');

-- Entity type filtering for different AI operations
CREATE INDEX idx_llm_event_record_entity_type_filter
ON scouter.llm_event_record (entity_type, space, name, version, created_at DESC);

-- UI pagination with consistent ordering
CREATE INDEX idx_llm_event_record_pagination 
ON scouter.llm_event_record (space, name, version, created_at DESC, uid);

-- JSONB indexes for efficient metadata and input querying
CREATE INDEX idx_llm_event_record_metadata_gin
ON scouter.llm_event_record USING GIN (metadata);

CREATE INDEX idx_llm_event_record_inputs_gin
ON scouter.llm_event_record USING GIN (inputs);

-- Update partition configuration
UPDATE scouter.part_config 
SET parent_table = 'scouter.llm_event_record'
WHERE parent_table = 'scouter.llm_drift_record';

-- Update cron jobs to reference new table name
UPDATE cron.job 
SET command = REPLACE(command, 'llm_drift_record', 'llm_event_record')
