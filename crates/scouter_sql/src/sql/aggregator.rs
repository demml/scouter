...
use scouter_types::{Attribute, TraceId, TraceSpanRecord, TraceSummaryRecord};
use sqlx::PgPool;
use std::sync::Arc;
...
mod tests {
    ...
    use scouter_types::{TraceId, TraceSpanRecord};

    #[test]
    fn aggregator_summary_does_not_carry_eval_profile_associations() {
        let span = TraceSpanRecord {
            trace_id: TraceId::from_bytes([7; 16]),
            ..Default::default()
        };
        let agg = TraceAggregator::new_from_span(&span);
        let summary = agg.to_summary_record();
        assert_eq!(summary.trace_id, span.trace_id);
    }
}
