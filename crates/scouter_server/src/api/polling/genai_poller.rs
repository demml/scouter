...
pub struct BackgroundGenAIDriftManager {
...
impl BackgroundGenAIDriftManager {
    pub async fn start_workers(
        ...
        info!("Starting {} GenAI drift workers", num_workers);
        let mut workers = Vec::with_capacity(num_workers);

        for id in 0..num_workers {
            let shutdown_rx = shutdown_rx.clone();
            let genai_poller = AgentPoller::new(
                db_pool,
    ...
    async fn start_worker(
        ...
        loop {
            tokio::select! {
                _ = shutdown.changed() => {
                    info!("GenAI evaluator {}: Shutting down", id);
                    break;
                }
                result = poller.poll_for_tasks().instrument(span!(Level::INFO, "poll_for_genai_tasks")) => {
                    if let Err(e) = result {
