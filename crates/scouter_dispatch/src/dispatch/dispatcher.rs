    ...
    use scouter_types::{DriftArgs, AlertDispatchType, DispatchAlertDescription, DispatchDriftConfig };
    ...
    impl OpsGenieAlerter {
        ...
        pub fn new(name: &str, repository: &str, version: &str) -> Result<Self, DispatchError> {
            let api_key = env::var("OPSGENIE_API_KEY").map_err(|_| {
                DispatchError::OpsGenieError("OPSGENIE_API_KEY is not set".to_string())
            })?;
    ...
    mod tests {
        ...
        async fn test_send_console_alerts() {
            let features = test_features_map();
            let dispatcher = AlertDispatcher::Console(ConsoleAlertDispatcher::new(
                "name",
                "repository",
                "1.0.0",
            ));
