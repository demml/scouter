...
impl TraceSpanBatchBuilder {
    ...
    fn append_links(&mut self, links: &[SpanLink]) -> Result<(), TraceEngineError> {
        ...
        for link in links {
            ...
            let mut trace_bytes = vec![0u8; 16];
            hex::decode_to_slice(&link.trace_id, &mut trace_bytes).map_err(|e| {
            ...
            let mut span_bytes = vec![0u8; 8];
            hex::decode_to_slice(&link.span_id, &mut span_bytes)
